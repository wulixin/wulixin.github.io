...
dataprices = pro(api_name="daily",trade_date='20230213')
...
#########################六大产业链
# 房地产产业链
FDC<-c("全国地产","区域地产","水泥","染料涂料","装修装饰","房产服务","建筑工程",
       "其他建材")
# 电子商务产业链
...
DJRstocks_name<-stocks_names%>%
  filter(industry %in% DJR)

DJRchina_stocks <- DJRstocks_name %>%
  mutate(
...
DJRstocks<-DJRchina_stocks%>%group_by(name)%>%
  mutate(return=(Close-Open)/Open)%>%
  ungroup(name)

DJRstocks$return_type<-cut(DJRstocks$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
...
DJRDT<-DJRstocks%>%
  ...
  arrange(desc(游资偏好))%>%
  left_join(df_all)%>%
  select(name,symbol,游资偏好,close,pct_chg,ShiZhi,GaiNian,大涨,
         CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)

colnames(DJRDT)<-c("公司名称","代码","游资偏好度","收盘价","涨跌幅","市值","大方向","爆发度","产业趋势",
...
#六大主流方向
addWorksheet(wb,sheetName="大健康")
addWorksheet(wb,sheetName="大消费")
addWorksheet(wb,sheetName="大金融")
addWorksheet(wb,sheetName="新能源")
addWorksheet(wb,sheetName="新基建")
addWorksheet(wb,sheetName="新(高)科技")
addWorksheet(wb,sheetName="ChatGPT国产替代")


#五大重要产业链
addWorksheet(wb,sheetName="房地产产业链")
addWorksheet(wb,sheetName="电子商务产业链")
addWorksheet(wb,sheetName="智能汽车产业链")
addWorksheet(wb,sheetName="特高压产业链")
addWorksheet(wb,sheetName="军工产业链")
addWorksheet(wb,sheetName="实体经济产业")
...
#六大主流方向
writeDataTable(wb,sheet="大健康",x=DJKDT,tableStyle = "TableStyleLight2",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="大消费",x=DXFDT,tableStyle ="TableStyleMedium9",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="大金融",x=DJRDT,tableStyle ="TableStyleMedium10",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="新能源",x=XNYDT,tableStyle ="TableStyleMedium4",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="新基建",x=XJJDT,tableStyle ="TableStyleMedium4",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="新(高)科技",x=XKJDT,tableStyle = "TableStyleLight2",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="ChatGPT国产替代",x=GCTD, tableStyle = "TableStyleLight6",withFilter = openxlsx_getOp("withFilter", TRUE))

# 五大重要产业链 
writeDataTable(wb,sheet="房地产产业链",x=FDCDT,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="电子商务产业链",x=DZSWDT,tableStyle ="TableStyleMedium7",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="智能汽车产业链",x=ZNQCDT,tableStyle ="TableStyleMedium2",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="特高压产业链",x=TGYDT,tableStyle ="TableStyleLight4",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="军工产业链",x=JGDT,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
writeDataTable(wb,sheet="实体经济产业",x=STJJDT,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
...
saveWorkbook(wb,"//Users//wulixin//Desktop//短线操盘葵花宝典六大主流方向.xlsx",overwrite=TRUE)
openXL("//Users//wulixin//Desktop//短线操盘葵花宝典六大主流方向.xlsx")}
