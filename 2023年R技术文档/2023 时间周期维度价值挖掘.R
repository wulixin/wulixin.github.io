...
dataprices = pro(api_name="daily",trade_date='20230317')
...
df_all$Year<-year(ymd(df_all$list_date))

stocks_names$Year<-year(ymd(stocks_names$list_date))

unique(stocks_names$Year)

{## 数据获取
  stocks_22<- stocks_names%>%
    filter(Year==2022)%>%
    mutate(
  ...
  stocks_22$return_type<-cut(stocks_22$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
  ...
  DT_22<-stocks_22%>%
    ...
    arrange(desc(游资偏好))%>%
    left_join(df_all)%>%
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_22)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                    "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }



{## 数据获取
  stocks_21<- stocks_names%>%
    filter(Year==2021)%>%
    mutate(
      stock.prices = map(ts_code, 
                         function(.x) get_stock_prices(.x)),
  ...
  stocks_21$return_type<-cut(stocks_21$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
                             labels =c("大跌","中跌","微跌","小涨","中涨","大涨","游资偏好"))
  DT_21<-stocks_21%>%
    filter(Date>ymd(Sys.Date()-300))%>%
    select(name,return_type)%>%
    ...
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_21)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                     "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }
...
{## 数据获取
  ...
  stocks_20$return_type<-cut(stocks_20$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
                             labels =c("大跌","中跌","微跌","小涨","中涨","大涨","游资偏好"))
  DT_20<-stocks_20%>%
    filter(Date>ymd(Sys.Date()-300))%>%
    select(name,return_type)%>%
    ...
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_20)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                     "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }
...
{## 数据获取
  ...
  stocks_19$return_type<-cut(stocks_19$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
                             labels =c("大跌","中跌","微跌","小涨","中涨","大涨","游资偏好"))
  DT_19<-stocks_19%>%
    filter(Date>ymd(Sys.Date()-300))%>%
    select(name,return_type)%>%
    ...
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_19)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                     "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }



{## 数据获取
  stocks_18<- stocks_names%>%
    filter(Year==2018)%>%
    mutate(
      stock.prices = map(ts_code, 
                         function(.x) get_stock_prices(.x)),
  ...
  stocks_18$return_type<-cut(stocks_18$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
                             labels =c("大跌","中跌","微跌","小涨","中涨","大涨","游资偏好"))
  DT_18<-stocks_18%>%
    filter(Date>ymd(Sys.Date()-300))%>%
    select(name,return_type)%>%
    ...
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_18)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                     "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }


{## 数据获取
  stocks_17<- stocks_names%>%
    filter(Year==2017)%>%
    mutate(
      stock.prices = map(ts_code, 
                         function(.x) get_stock_prices(.x)),
  ...
  stocks_17$return_type<-cut(stocks_17$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
                             labels =c("大跌","中跌","微跌","小涨","中涨","大涨","游资偏好"))
  DT_17<-stocks_17%>%
    filter(Date>ymd(Sys.Date()-300))%>%
    select(name,return_type)%>%
    ...
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_17)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                     "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }
...
{## 数据获取
  ...
  stocks_16$return_type<-cut(stocks_16$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
                             labels =c("大跌","中跌","微跌","小涨","中涨","大涨","游资偏好"))
  DT_16<-stocks_16%>%
    filter(Date>ymd(Sys.Date()-300))%>%
    select(name,return_type)%>%
    ...
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_16)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                     "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }



{## 数据获取
  stocks_15<- stocks_names%>%
    filter(Year==2015)%>%
    mutate(
      stock.prices = map(ts_code, 
                         function(.x) get_stock_prices(.x)),
  ...
  stocks_15$return_type<-cut(stocks_15$return, breaks=c(-0.1,-0.05,-0.015,0,0.025,0.035,0.08,0.12), 
                             labels =c("大跌","中跌","微跌","小涨","中涨","大涨","游资偏好"))
  DT_15<-stocks_15%>%
    filter(Date>ymd(Sys.Date()-300))%>%
    select(name,return_type)%>%
    ...
    select(name,symbol,游资偏好,大涨,close,pct_chg,ShiZhi,Year,
           CY_FaZhanQS,CY_JiShuXJ,CY_ChanPinGQ,CY_ChanYeType,area,market,industry,Time)
  colnames(DT_15)<-c("公司名称","代码","游资偏好度","爆发力","收盘价","涨跌幅","市值","上市年份","产业趋势",
                     "产业技术","产品类型","产业类型","区域","板块","行业","上市天数") }
...
{library(officer)
  ...
  wb<-createWorkbook()
  #五大重要产业链
  addWorksheet(wb,sheetName="2022年新股")
  addWorksheet(wb,sheetName="2021年新股")
  addWorksheet(wb,sheetName="2020年新股")
  addWorksheet(wb,sheetName="2019年新股")
  addWorksheet(wb,sheetName="2018年新股")
  addWorksheet(wb,sheetName="2017年新股")
  addWorksheet(wb,sheetName="2016年新股")
  addWorksheet(wb,sheetName="2015年新股")
  
  #insertImage(wb, "小美女","//Users//wulixin//Desktop//kehu.png", startRow = 10, startCol = 5, width = 6, height = 5)
  # 五大重要产业链 
  writeDataTable(wb,sheet="2022年新股",x=DT_22,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
  writeDataTable(wb,sheet="2021年新股",x=DT_21,tableStyle ="TableStyleMedium7",withFilter = openxlsx_getOp("withFilter", TRUE))
  writeDataTable(wb,sheet="2020年新股",x=DT_20,tableStyle ="TableStyleMedium2",withFilter = openxlsx_getOp("withFilter", TRUE))
  writeDataTable(wb,sheet="2019年新股",x=DT_19,tableStyle ="TableStyleLight4",withFilter = openxlsx_getOp("withFilter", TRUE))
  writeDataTable(wb,sheet="2018年新股",x=DT_18,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
  writeDataTable(wb,sheet="2017年新股",x=DT_17,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
  writeDataTable(wb,sheet="2016年新股",x=DT_16,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
  writeDataTable(wb,sheet="2015年新股",x=DT_15,tableStyle ="TableStyleMedium6",withFilter = openxlsx_getOp("withFilter", TRUE))
  
  saveWorkbook(wb,"//Users//wulixin//Desktop//周期之庄家坐庄战争需要打多久.xlsx",overwrite=TRUE)
  openXL("//Users//wulixin//Desktop//周期之庄家坐庄战争需要打多久.xlsx")}
